...
from xlviews.utils import label_func_from_list, format_label
...
def split_style(handle):
    ...
    if isinstance(handle, tuple):
        ...
        if isinstance(style, dict):
            style = {key if isinstance(key, tuple) else (key,): value
                     for key, value in style.items()}
        elif isinstance(style, list):
            style = [key if isinstance(key, tuple) else (key,)
                     for key in style]
        return handle, style
    else:
        return handle, {}
...
def autohandle(func):
    ...
    """
    @wraps(func)
    def _func(self, *args, by=None, marker=None, color=None, alpha=None,
              **kwargs):
        marker, marker_style = split_style(marker)
        color, color_style = split_style(color)
        alpha, alpha_style = split_style(alpha)
        self.set_style(marker=marker_style, color=color_style,
                       alpha=alpha_style)
        ...
        by, marker, color, alpha = combine_handle(by, marker, color,
...
class Element(object):
    ...
    def set_output(self, sheet=None, left=None, top=None, width=None,
                   height=None, row=None, column=None,
    ...
    def set_axis(self, xticks='auto', yticks='auto', xlabel='auto',
                 ylabel='auto', xformat='auto', yformat='auto', xscale=None,
                 yscale=None):
        axes = self.axes
        if xscale:
    ...
    @autohandle
    @autolegend
    def _plot(self, x, y, chart_type=None, by=None, label=None, sel=None,
              sheet=None, left=None, top=None, width=None, height=None,
              row=None, column=None, axes=None, title='auto', line=False,
              line_width=False, name='{title_}_{x_}_{y_}',
              include_in_layout=False, **kwargs):
        """
        プロット本体
        ...
        if axes or self.axes:
            ...
            self.set_output(sheet=sheet, left=left, top=top, width=width,
        ...
        if (data.columns_level == 1 and
                (x not in columns or y not in columns) and
                not isinstance(x, tuple) and not isinstance(y, tuple)):
            ...
            for k in range(len(data)):
                row = start + k
                if sel[k]:
                    key = data.sheet.range((row, column),
                                           (row, column +
                                            data.index_level)).value
                    series_name = get_label_name(label, by, key,
                                                 **self.const_dict)
                    by_key = dict(zip(by, key))
                    if header == 'both':
                        # XValues
                        index = data.sheet.range((row, index_x[0]),
                                                 (row, index_x[-1]))
                        # Values
                        columns = data.sheet.range((row, index_y[0]),
                                                   (row, index_y[-1]))
                    elif header == 'x':
        ...
        if is_new_axes:
            ...
            if title:
                ...
                if isinstance(title, str) or callable(title):
                    title = format_label(data, title, sel,
                                         default=self.const_dict)
                else:
                    name = None
                if title != 'auto':
    ...
    def add_axes(self):
        self.axes = Axes(sheet=self.sheet, left=self.left, top=self.top,
    ...
    def add_series(self, index, columns, name, axis=1):
        ...
        series = self.axes.add_series(index=index, columns=columns,
                                      name=name, axis=axis, sheet=sheet,
    ...
    def series_styles(self):
        ...
        return [dict(zip(handles, style)) for style in zip(*styles)]
    ...
    def _series_styles(self, handle):
        ...
        if not isinstance(style, dict):
            style = dict(zip(style, palette(handle, len(style))))
        for series, by_key, default in zip(self.series_collection,
                                           self.by_key, defaults):
    ...
    def apply_style(self, **kwargs):
        for series, style in zip(self.series_collection, self.series_styles):
    ...
    def bar(self, x, y, label='auto', stacked=False, **kwargs):
        ...
        for y in ys:
            if label == 'auto' and len(ys) > 1:
                ...
                label_ = label
            self._plot(x, y, chart_type=chart_type, label=label_,
                       **kwargs)

        if xs:
            sheet = self.data.sheet
            xvalues = sheet.range(self.data.range(xs[0])[0],
