"""
エクセルシートのテーブルをPandasのDataFrameと連携する．

カラムに名前があるときは，インデックスが無名の場合を除き，無視される．
"""
import re
...
from xlviews.style import (get_number_format, set_alignment, set_border,
                           set_fill, set_font, set_frame_style,
                           set_number_format, set_table_style)
from xlviews.utils import (add_validation, array_index, columns_list,
                           get_sheet_cell_row_column, multirange)


class SheetFrame(object):
    """
    ...
    @wait_updating
    def __init__(self, *args, data=None, index=True, index_level=1,
                 columns_level=1, sort_index=False, parent=None, head=None,
                 style=True, gray=False, autofit=True, number_format=None,
                 name=None, font_size=None, **kwargs):
        """
        ...
        if data is not None:
            ...
            if style:
                self.set_style(gray=gray, autofit=autofit, font_size=font_size,
                               **kwargs)
            if head is None:
            ...
            for table in self.sheet.api.ListObjects:
                if (table.Range.Row == self.row and
                        table.Range.Column == self.column):
                    self.table = table
                    break
    ...
    def index(self, column, relative=False):
        ...
        if isinstance(column, list):
        ...
        elif self.columns_level > 1:  # 積層カラムのカラム名指定
            ...
            if isinstance(column, tuple):
                ...
                values = self.sheet.range(
                    ...
                    self.cell.offset(0, end - 1)).value
    ...
    def __getitem__(self, column):
        ...
        if column == slice(None, None, None):
            ...
            return df
        elif isinstance(column, str) or isinstance(column, tuple):
    ...
    def columns(self):
        ...
        if self.columns_level == 1:
            ...
            columns_ = [tuple(column) for column in zip(*columns_)]
            return columns_
    ...
    def range(self, column=None, start=None, end=None):
        ...
        if column is None:
            start = self.cell
            end = start.offset(self.columns_level + len(self) - 1,
                               self.index_level + len(self.value_columns) - 1)
            return self.sheet.range(start, end)
        elif self.columns_level == 1 or isinstance(column, tuple):
            if start == 0:
                start = self.row
            ...
            if end is None:
                ...
                return self.sheet.range(start, end)
        elif self.index_level == 1 and column == 'index':
            if start == 0:
                start = self.cell
                end = start.offset(self.columns_level - 1)
            elif start is None:
    ...
    def groupby(self, by, sel=None):
        ...
        if by is None:
            ...
            if self.columns_names is None:
                ...
                df = pd.DataFrame(self.value_columns,
    ...
    def set_number_format(self, *number_format, autofit=False, split=True,
                          **columns_format):
        if number_format:
            ...
            if isinstance(number_format, dict):
                ...
                cell = xw.Range(self.cell.offset(self.columns_level,
                                                 self.index_level),
                                self.range()[-1])
        ...
        if self.columns_level == 1:
            for column in chain(self.columns, self.wide_columns):
                if (column in self.columns and
                        isinstance(column, str) and split):
    ...
    def set_style(self, columns_alignment=None, gray=False, **kwargs):
        set_frame_style(self.cell, self.index_level,
                        self.columns_level, len(self),
                        len(self.value_columns), gray=gray, **kwargs)
        ...
        for wide_column in wide_columns:
            ...
            set_border(range_, edge_width=edge_width, inside_width=1,
        ...
        for wide_column in wide_columns:
            ...
            set_border(range_, edge_width=edge_width, inside_width=None,
                       edge_color=edge_color)
            if gray:
                set_font(range_, color='#aaaaaa')
    ...
    def astable(self, header=True, autofit=False):
        ...
        table = self.sheet.api.ListObjects.Add(xlsrcrange, table.api, None,
    ...
    def autofilter(self, *args, **field_criteria):
        ...
        for field, criteria in field_criteria.items():
            ...
            if isinstance(criteria, list):
                ...
                filter_(Field=field, Criteria1=criteria,
                        Operator=operator.xlFilterValues)
            elif isinstance(criteria, tuple):
                filter_(Field=field, Criteria1=f'>={criteria[0]}',
    ...
    @wait_updating
    def copy(self, *args, columns=None, n=1, header_ref=False,
             sort_index=False, sel=None, rows=None, drop_duplicates=False,
             autofit=True, **kwargs):
        """
        自分の参照コピーを作成する．
        ...
        *args :
        ...
        cell.value = values
        sf = SheetFrame(cell, index_level=index_level, autofit=False,
                        **kwargs)

        self_columns = self.columns
        number_format = {column: self.get_number_format(column)
                         for column in columns if column in self_columns}
    ...
    def add_wide_column(self, column: str, values, number_format=None,
    ...
    def add_validation(self, ref: str, column=None, name='valid',
                       invalid='""', valid_value='○', invalid_value='×',
                       default=True):
        """
        ...
        if name not in self.columns:
            ...
            add_validation(self.range(name, -1), [valid_value, invalid_value],
                           default=default)
        number_format = self.get_number_format(ref)
        self[column] = (f'=IF({{{name}}}="○",{{{ref}}},{invalid})',
                        number_format, True)
        self.set_style()
...
def main():
    ...
    df = data.get(['wafer', 'sx', 'sy', 'cad', 'Rmin',
                   'Rmax', 'TMR', 'Hc', 'Hc'],
                  sy=2, cad='Y60')
